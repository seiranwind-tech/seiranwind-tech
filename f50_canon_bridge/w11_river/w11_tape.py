...
trk = np.zeros((T, N), np.int32); Z = []; Cs = {}; t0 = time.time()
for t in range(T):
    eng.step()
    trk[t] = eng.centre_track_ids[eng.labels]
    if (t + 1) % 5 == 0:
        Z.append(eng.zin.astype(np.float16)); Cs[t] = (eng.centre_track_ids.copy(), eng.centres.astype(np.float16).copy())
    if t % 500 == 0: print(t, len(eng.centres), eng.lineage_counts, round(time.time() - t0), flush=True)
out = os.path.join(os.environ["W11_TAPES"], f"tape_{block}_{perm}_{seed}.npz")
ct = np.array(sorted(Cs)); np.savez(out, trk=trk, Z=np.stack(Z), ctimes=ct,
