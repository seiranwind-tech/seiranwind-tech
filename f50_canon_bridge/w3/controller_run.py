...
def build_moves_map(moves, step_offset=0):
    """Map a move's 'step' (the reassignment-completed step) to the
    (seed, basin, redraw_index) key used elsewhere. step_offset=0 matches
    the PU-bridge convention (redraw_index = step // REDRAW, i.e. moves
    step values are 0-indexed reassign steps t=49,99,...). step_offset=1
    matches the canonical-bridge convention (redraw_index = (step-1) //
    REDRAW, i.e. table's own redraw_index/phase columns, phase 49 at the
    reassignment step; verified against w1/DEV_table.npz's redraw_index
    column for the canonical line)."""
    mv = {}
    for m in moves:
        r = (m["step"] - step_offset) // REDRAW
...
def trace(table_npz_path, events_json_path, freeze_path=None):
    ...
    mv = build_moves_map(moves, step_offset=freeze.get("step_offset", 0))
